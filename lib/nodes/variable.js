...
module.exports = (node) => {
  ...
  // LESS variables are @name : value; < note that for them to be valid LESS vars, they must end inspect
  // a semicolon.

  // console.log(node);
  //
  // const [rule] = node.source.input.css.split(';');
  // const [ruleLine] = rule.split('\n');
  // const [nodeSource] = ruleLine.split(':');
  //
  // if (bracketsPattern.test(nodeSource)) {
  //   return;
  // }
