...
const toString = require('./nodes/toString');

const importantPattern = /(\s*!\s*important)$/i;
...
module.exports = class LessParser extends Parser {
  ...
  unknownWord(tokens) {
    ...
    if (isMixinToken(first)) {
      ...
      if (importantMatch) {
        const [, importantString] = importantMatch;

        this.lastNode.important = true;
        this.lastNode.params = this.lastNode.params.replace(importantString, '');

        if (!params.includes(' !important')) {
          this.lastNode.raws.important = importantString;
        }
      }
