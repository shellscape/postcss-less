...
export default function lessTokenize (input) {
    ...
    while (pos < length) {
        ...
        switch (symbolCode) {
            ...
            {
                if (symbolCode === openedСurlyBracket) {
                    openedBlocksCount++;
                } else if (symbolCode === closedCurlyBracket) {
                    openedBlocksCount--;
                } else if (symbolCode === closedParentheses) {
            ...
            case ampersand:
            {
                ...
                if (openedBlocksCount > 0 && css.slice(nextPos, nextPos + extendSelectorLength) === extendSelector) {

                    //skip inner extend
                    nextPos = css.indexOf(symbol, nextPos + extendSelectorLength) + 1;

                    tokens.push(['extend', css.slice(pos, nextPos),
                        line, pos - offset,
                        line, nextPos - offset
                    ]);
