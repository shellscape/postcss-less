...
export default function lessTokenize(input) {
  ...
  while (pos < length) {
    ...
    const symbol = css[pos];
    let nextSymbolCode = null;
    let nextPos = null;
    let escaped = null;
    let lines = null;
    let lastLine = null;
      ...
      case openedParenthesis:
        {
          ...
          if (
            ...
            tokens.push([
              'brackets', css.slice(pos, nextPos + 1),
              line, pos - offset,
              line, nextPos - offset,
            ]);
            pos = nextPos;
            ...
            const badBracket = badBracketPattern.test(cssPart);
            const foundParam = cssPart.indexOf('@') >= 0;
            const foundString = /['"]/.test(cssPart);
      ...
      case doubleQuote:
        {
          ...
          tokens.push([
            'string', css.slice(pos, nextPos + 1),
            line, pos - offset,
            line, nextPos - offset,
          ]);
          pos = nextPos;
      ...
      case atRule:
        {
          ...
          if (css.charCodeAt(pos + 1) === openedCurlyBracket) {
            ...
            if (variablePattern.test(cssPart)) {
              ...
              const nParenStart = tokens.findIndex((x) => x[0] === '(');
              const nParenEnd = tokens.findIndex((x) => x[0] === ')');
            ...
            if (token === 'mixin-param') {
              const varDictPos = cssPart.indexOf('...');
      ...
      default:
        {
          ...
          if (symbolCode === ampersand && css.slice(pos, pos + nestedExtendWordLength) === nestedExtendWord) {
            ...
            if (end !== -1) {
              tokens.push([
                'nested-extend', css.slice(pos, end + 1),
                line, pos - offset,
          ...
          if (symbolCode === slash && nextSymbolCode === asterisk) {
            ...
            if (symbolCode === tilde) {
              ...
              const match = quotePattern.exec(css);

              if (match && match.index === pos + 1) {
                const end = findEndOfExpression(css, length, quotePattern.lastIndex + 1);
